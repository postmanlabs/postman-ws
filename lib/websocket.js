 ...
 * @returns {Object[]}
 */
function _parseRequestHeaders (headerString) {
 ...
 * @returns {Object[]}
 */
 function _parseResponseHeaders (rawHeaders) {
  let acc = []

  for (let i = 0, ii = rawHeaders.length; i < ii; i = i + 2) {
    acc.push({
      key: rawHeaders[i],
...
function initAsClient(websocket, address, protocols, options) {
  ...
  _reqResInfo.request = {
    method: req.method,
    href: `${isSecure ? 'https' : 'http'}://${parsedUrl.host}${parsedUrl.pathname}${parsedUrl.search}`,
    headers: req._header ? _parseRequestHeaders(req._header) : [],
    httpVersion: '1.1'
  };

  websocket._debug.push(_reqResInfo);
  ...
  req.on('response', (res) => {
    ...
    _reqResInfo.response = _reqResInfo.response || {
      statusCode: res.statusCode,
      statusMessage: res.statusMessage,
      headers: _parseResponseHeaders(res.rawHeaders),
